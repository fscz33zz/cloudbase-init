...
class DisableRootPlugin(base.BaseCloudConfigPlugin):
    """Sets passwords for users given in the cloud-config format."""

    def __init__(self):
        self._osutils = osutils_factory.get_os_utils()

    def _ssh_allow_root_login(self, answer):
        with open("C:/ProgramData/ssh/sshd_config", 'r') as f:
            sshd_config = f.read()
        sshd_config_new = re.sub(".*PermitRootLogin.*",
                                 "PermitRootLogin %s" % answer,
                                 sshd_config)
    ...
    def process(self, data):
        ...
        self._ssh_allow_root_login("no" if data else "yes")
